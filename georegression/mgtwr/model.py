...
class GWR(BaseModel):
    ...
    def _local_predict(self,predict_X,i):
        wi = self._build_wi(i, self.bw).reshape(-1, 1)
        betas, inv_xtx_xt = _compute_betas_gwr(self.y, self.X, wi)
        predict = np.dot(predict_X[i], betas)[0]
    ...
    def predict(self,coor,predict_X):
        predict_X = np.hstack([np.ones((predict_X.shape[0], 1)), predict_X])
        result = [self._local_predict(predict_X,i) for i in range(predict_X.shape[0])]
...
class GTWR(BaseModel):
    ...
    def predict(self, coor, t , predict_X):
        predict_X = np.hstack([np.ones((predict_X.shape[0], 1)), predict_X])
        result = [self._local_predict(predict_X, i) for i in range(predict_X.shape[0])]
